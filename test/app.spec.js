...
const makeSuggestionsArray = require("./suggestions.fixtures");
...
describe("Suggestions Endpoints", function () {
  ...
  afterEach("cleanup", () =>
    db.raw("TRUNCATE suggestions RESTART IDENTITY CASCADE")
  );

  describe(`GET /api/suggestions`, () => {
    context(`Given no suggestions`, () => {
      it(`responds with 200 and an empty list`, () => {
        return supertest(app).get("/api/suggestions").expect(200, []);
      });
    });
    ...
    context("Given there are suggestions in the database", () => {
      ...
      beforeEach("insert suggestions", () => {
        return db.into("suggestions").insert(testSuggestions);
      });
  ...
  describe(`POST /api/suggestions`, () => {
    it(`creates a suggestion, responding with 201 and the new suggestion`, () => {
      ...
      return supertest(app)
        ...
        .then((res) =>
          supertest(app).get(`/api/suggestions/${res.body.id}`).expect(res.body)
        );
    });
  ...
  describe(`DELETE /api/suggestions/:suggestionId`, () => {
    ...
    context("Given there are suggestions in the database", () => {
      ...
      it("responds with 204 and removes the suggestion", () => {
        const idToRemove = 2;
        const expectedSuggestion = testSuggestions.filter(
          (suggestion) => suggestion.id !== idToRemove
        );
        return supertest(app)
          .delete(`/api/suggestions/${idToRemove}`)
          .expect(204)
          .then((res) =>
            supertest(app).get(`/api/suggestions`).expect(expectedSuggestion)
          );
      });
    });
  });
      ...
      it(`responds with 400 when no required fields supplied`, () => {
        ...
        return supertest(app)
          ...
          .expect(400, {
            error: {
              message:
                "Request body must contain either 'id', 'userid', 'title', 'content', 'date_published', 'date_modified', 'approved', 'date_approved', 'upvotes'",
            },
          });
      });
